  ...
  factory: (require) => {
    ...
    const MESSAGES = {
      zh: {
        ...
        modelStatsHitUnreportedTip: '该模型多次长上下文调用而缓存命中恒为 0:大概率是中转/代理层剥掉了缓存字段(或致真实未命中),命中率无法统计,金额按全未命中价计(上界)。直连官方 API 可恢复准确命中与更低金额。',
        ...
        modelStatsNote: '口径:命中率 = 缓存读 ÷ (缓存读 + 非缓存输入);「未上报?」= 用量疑似无缓存字段(常见于中转/代理),金额按全未命中计(上界);单价 = 费用 ÷ 总 token × 1M(USD/M);性价比 = 总 token ÷ 费用(越高越好)。费用按调用时美元价计。',
        ...
        planStatsNote: '口径:「金额」为目录 API 价等值(订阅耗套餐额度,预算只计 API);估算只含 dsh 内调用,服务端「已用%」为全账号口径;每 1%/满窗按可信采样段差分(短段波动标「精度受限」);5h 滚动,周/月按本地日历。',
        ...
        unmatchedHint: '按当前匹配模式未命中价格条目(含跨厂商兜底默认价);指定条目即写入手动匹配覆盖,选「默认价」回退 DeepSeek 默认价。本地推理(lmstudio / ollama / vLLM 等)默认零消耗,计费需在此指定条目。',
        ...
        qwenRatesAdd: '添加',
        qwenLocalNote: '本地账本估算，Credits 以控制台为准。仅统计 qwen、qwen-tokenplan、qianwen-tokenplan、qwen-token-plan、qianwen-token-plan 订阅渠道(含 llm- 前缀)；显式 API 分类与 qianwen 按量渠道不计。表外模型需手动指定抵扣率。',
        ...
        volcengineNote: '火山方舟需 AK/SK HMAC 签名(非 ARK_API_KEY):IAM 子用户授予 ArkReadOnlyAccess + BillingCenterReadOnlyAccess 后填 AccessKeyID/SecretAccessKey,或导出 VOLC_ACCESSKEY/VOLC_SECRETKEY 自动发现;凭据仅发往 open.volcengineapi.com。',
        ...
        cardsFootnote: '口径:金额为最近落盘快照(2 秒防抖,关闭即落盘),与官方实时账单的分钟级时差属预期;tokens 列为输入+输出+缓存+推理合计,推理 tokens 不计费(官方账单并入「输出」列),对账以金额为准。',
        timezoneHint: '时区提示:「今日/本月」按宿主机时区 {host} 取日界,与当前浏览器({browser})不同时,本地午夜后的调用会记到前一日(并非漏计;官方余额对账也会提示偏差)。可在宿主机以本地时区运行 dsh 对齐。',
        todaySessions: '今日会话',
        ...
        priceTableNote: '价格按调用时间选择:Flash 新价自 2026-09-10 12:00(北京)生效,V4 Pro 自 09-14 12:00 转按 Flash 计价,此前历史价保留;工作日高峰 09–12、14–18 时,峰价为谷价两倍,周末全天谷价;缓存写入按命中价计。',
        ...
        syncScopeNote: '同步范围:「从官方文档同步价格」仅更新 DeepSeek 官方模型价(含峰谷档);z-ai / Kimi / Qwen / Volcano 等第三方目录价随版本维护,滞后可在「拓展价格表」编辑,或用「模型名匹配」定价。',
      ...
      en: {
        ...
        modelStatsHitUnreportedTip: 'Many long-context calls with zero cache hits: most likely a relay/proxy strips cache fields from usage (or genuinely breaks the prefix cache). Hit rate cannot be computed; cost is all-cache-miss (an upper bound). Direct official API access restores accurate hits and lower costs.',
        ...
        modelStatsNote: 'Hit rate = cache reads ÷ (reads + non-cached input); "unreported?" = usage has no cache fields (common behind relays), cost priced all-miss (upper bound); blended = cost ÷ tokens × 1M; value = tokens ÷ cost (higher better). Costs use call-time USD prices.',
        ...
        planStatsNote: 'Scope: amounts are catalog-price equivalents (plans consume quota; budget counts API only). Estimates cover dsh calls only; server "used %" is account-wide. Per-1%/full-window from trusted sample deltas (short segments tagged low-precision). 5h rolling, week/month local calendar.',
        ...
        unmatchedHint: 'No price entry hit for the current match mode (incl. cross-vendor fallback to the DeepSeek default). Pick the entry each bills against (saved as manual override); “Default price” = DeepSeek default. Local-runtime models (lmstudio / ollama / vLLM / …) are zero-cost unless assigned here.',
        ...
        qwenRatesAdd: 'Add',
        qwenLocalNote: 'Local estimate; console is authoritative. Only qwen, qwen-tokenplan, qianwen-tokenplan, qwen-token-plan and qianwen-token-plan providers count (llm- prefix ok). Explicit API classifications and the qianwen pay-as-you-go provider are excluded; add rates for models outside the built-in table.',
        ...
        volcengineNote: 'Volcano Ark needs AK/SK HMAC signature (not ARK_API_KEY): IAM sub-user with ArkReadOnlyAccess + BillingCenterReadOnlyAccess, then enter AccessKeyID/SecretAccessKey; or export VOLC_ACCESSKEY/VOLC_SECRETKEY (official SDK env); credentials only go to open.volcengineapi.com.',
        volcengineAccessKeyPlaceholder: 'AK...',
        volcengineSecretPlaceholder: 'SK...',
        ...
        cardsFootnote: 'Amounts are the latest ledger snapshot (2s debounce; shutdown flushes) — minute lag vs the live bill is expected. Tokens sum input+output+cache+reasoning; reasoning is unbilled (folded into output on the official bill) — reconcile by amount.',
        ...
        priceTableNote: 'Prices follow the call timestamp: Flash rates from 04:00 UTC Sep 10 2026; V4 Pro to Flash rates from 04:00 UTC Sep 14; earlier rates remain for history. Weekday peaks 01–04 & 06–10 UTC at twice off-peak; weekends off-peak. Cache writes use the hit rate.',
        ...
        syncScopeNote: 'Sync scope: "Sync prices from official docs" only updates DeepSeek official prices (incl. peak/off-peak). Third-party catalog prices (z-ai / Kimi / Qwen / Volcano) follow plugin releases; edit in the extended catalog or match by name.',
