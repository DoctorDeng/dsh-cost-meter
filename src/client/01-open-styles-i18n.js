  ...
  factory: (require) => {
    ...
    const MESSAGES = {
      zh: {
        ...
        modelStatsHitUnreportedTip: '长上下文调用始终无缓存命中：代理可能剥掉缓存字段或破坏缓存。命中率未知，按全未命中价估算金额上界；直连官方 API 可核对。',
        ...
        priceTableNote: '按调用时间计价：Flash 新价自 9 月 10 日北京 12:00 生效，V4 Pro 自 9 月 14 日同刻转按 Flash 价；此前保留历史价。周一至周五 09–12、14–18 为两倍峰价，周末及中国公众假期全天谷价；缓存写入按命中价。',
        ...
        legacyImportNote: '首次启动自动导入安装前历史；拷入旧日志后可在此重跑。金额按当前价格回推；已删除日志无法回放，跨安装时刻的会话不计安装前用量。',
        confirmClear: '确认清除',
      ...
      en: {
        ...
        aliyunBalanceNote: 'Shows Alibaba Cloud China funds in the returned currency. Requires a RAM AccessKey with bss:DescribeBillingAccount, not a Qianwen API key. First two fields are required; Security Token is for STS only. Secrets stay in the credential store.',
        ...
        customBalanceHeadersVarNote: 'Use {{VAR_NAME}} for a DSH credential or env variable without storing its value. <ROUTE>_API_KEY uses the uppercase Provider ID with punctuation replaced by underscores (openai→OPENAI_API_KEY, abc23-d→ABC23_D_API_KEY). Same name shares one key.',
        ...
        goQuotaDisabledNote: 'Enable to read OpenCode Go quota (rolling 5h, weekly, monthly) in the sidebar, Settings and bottom-right corner. If no subscription exists, the reason appears here.',
        ...
        catalogNote: 'Read-only prices by vendor and model family. Mount an entry to bill with it. Unmounting a DeepSeek model restores the default price; it can be mounted again.',
        ...
        modelStatsHitUnreportedTip: 'Long calls with zero hits may mean a relay stripped cache fields or broke prefix caching. Hit rate is unknown; all-miss cost is an upper bound. Direct API access may restore accurate hits.',
        ...
        modelStatsNote: 'Hit rate = cache reads ÷ (reads + uncached input). “Unreported?” means missing cache fields, often behind relays; all-miss cost is an upper bound. Unit cost = cost ÷ tokens × 1M; value = tokens ÷ cost. Uses call-time USD prices.',
        ...
        planStatsNote: 'Amounts use catalog-price equivalents; plans consume quota, budget counts API only. Estimates cover dsh calls; server “used %” is account-wide. Per-1% and full-window estimates use trusted sample deltas (short spans have limited precision). 5h rolling; week/month use local dates.',
        catalogDisplayLabel: 'Show directly in Cost settings',
        catalogDisplayHint: 'Controls direct display in the Cost price table; mounting and billing are unchanged. Hidden mounted models remain editable in this catalog’s vendor section.',
        priceTableDisplayHint: 'Use “Show directly in Cost settings” per model below. DeepSeek models appear here by default; mounted third-party models stay in the extended catalog.',
        ...
        unmatchedHint: 'No price matched, including the DeepSeek fallback. Choose an entry for manual billing; “Default price” uses the DeepSeek default. Local models (lmstudio, ollama, vLLM) cost zero until assigned.',
        ...
        matchedHint: 'These models matched automatically. Choose another entry or “Local model (zero cost)”. Local tokens still count; costs become zero, including history. Overrides move to the manual list above.',
        ...
        qwenRatesAdd: 'Add',
        qwenLocalNote: 'Local estimate; console is authoritative. Counts qwen, qwen-tokenplan, qianwen-tokenplan, qwen-token-plan and qianwen-token-plan (llm- prefix allowed). Excludes API routes and pay-as-you-go qianwen. Add rates for unlisted models.',
        ...
        scnetLocalNote: 'Accepts console snapshots up to 24 hours old. Otherwise estimates credits from local records and official rates. Other tools and unlisted models are excluded; check the console for actual usage.',
        ...
        cardsFootnote: 'Amounts use the latest ledger snapshot (2s debounce; flush on shutdown), so the bill may lag by minutes. Tokens include input, output, cache and reasoning. Reasoning is unbilled here and appears in official output; reconcile by amount.',
        ...
        priceTableNote: 'Prices follow call time: Flash rates since Sep 10 04:00 UTC; V4 Pro uses Flash rates since Sep 14 04:00 UTC. Earlier prices remain for history. Mon–Fri peaks 01–04 and 06–10 UTC cost twice off-peak; weekends and Chinese public holidays are off-peak. Cache writes use hit rates.',
        ...
        legacyImportNote: 'Pre-install history imports at first launch; rerun here after copying old logs. Costs use current prices. Deleted logs cannot be replayed; earlier usage in a session spanning installation is excluded.',
