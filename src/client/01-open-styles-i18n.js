  ...
  factory: (require) => {
    ...
    const MESSAGES = {
      zh: {
        ...
        unmatchedHint: '按当前匹配模式未命中价格条目(含跨厂商兜底后仍按默认价计的);指定条目即写入手动匹配覆盖,选「默认价」回退 DeepSeek 默认价。本地推理(lmstudio / ollama / vLLM 等)默认零消耗,需计费可在此指定条目。',
        ...
        qwenSource: '额度来源', qwenSourceLocal: '本地估算', qwenSourceCli: '官方 CLI',
        qwenCliNote: 'CLI 模式需在 DSH 主机安装 @qianwenai/qianwen-cli，并以同一系统账号运行 qianwen auth login。查询失败时可切回本地估算。',
        ...
        cardsFootnote: '口径:金额为最近落盘快照(2 秒防抖;正常关闭先强制落盘),与官方实时账单的分钟级时差属预期;tokens 列为输入+输出+缓存+推理合计,推理 tokens 不计费(官方账单并入「输出」列),对账以金额为准。',
        ...
        priceTableNote: '价格按调用时间选择:Flash 新价自 2026-09-10 12:00(北京)生效,V4 Pro 自 09-14 12:00 转按 Flash 计价,此前历史价保留;工作日高峰 09–12、14–18 时,峰价为谷价两倍,周末全天谷价;缓存写入按命中价计。设置自动保存。',
        ...
        syncScopeNote: '同步范围:「从官方文档同步价格」仅更新 DeepSeek 官方模型价(含峰谷档);z-ai / Kimi / Qwen / Volcano 等第三方目录价随插件版本维护,滞后可在「拓展价格表」挂载后编辑,或用「模型名匹配」手动定价。',
      ...
      en: {
        ...
        unmatchedHint: 'No price entry hit under the current match mode (incl. cross-vendor fallback to the DeepSeek default). Pick which entry each bills against (saved as a manual override); “Default price” = DeepSeek default. Local-runtime models (lmstudio / ollama / vLLM / …) are zero-cost unless assigned here.',
        ...
        qwenSource: 'Quota source', qwenSourceLocal: 'Local estimate', qwenSourceCli: 'Official CLI',
        qwenCliNote: 'Install @qianwenai/qianwen-cli on the DSH host and run qianwen auth login as the same OS user. Select local estimates if the CLI is unavailable.',
        ...
        cardsFootnote: 'Amounts are the latest ledger snapshot (2s debounce; clean shutdown flushes first) — minute-level lag vs the live bill is expected. Tokens sum input+output+cache+reasoning; reasoning is unbilled (folded into output on the official bill) — reconcile by amount.',
        ...
        priceTableNote: 'Prices follow the call timestamp: Flash rates from 04:00 UTC Sep 10 2026; V4 Pro to Flash rates from 04:00 UTC Sep 14; earlier rates remain for history. Weekday peaks 01–04 & 06–10 UTC at twice off-peak; weekends off-peak. Cache writes use the hit rate. Auto-saved.',
        ...
        syncScopeNote: 'Sync scope: "Sync prices from official docs" only updates DeepSeek official prices (incl. peak/off-peak). Third-party catalog prices (z-ai / Kimi / Qwen / Volcano) change per plugin release; adjust in the extended catalog or match by model name.',
