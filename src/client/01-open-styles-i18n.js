  ...
  factory: (require) => {
    ...
    const MESSAGES = {
      zh: {
        ...
        modelStatsNote: '口径:命中率 = 缓存读 ÷ (缓存读 + 非缓存输入);「未上报?」= 用量无缓存字段(常见于中转/代理),金额按全未命中计(上界);单价 = 费用 ÷ 总 token × 1M(USD/M);性价比 = 总 token ÷ 费用(越高越好)。',
      ...
      en: {
        ...
        modelStatsNote: 'Hit rate = cache reads ÷ (reads + non-cached input); "unreported?" = usage has no cache fields (common behind relays), cost priced all-miss (upper bound); blended = cost ÷ tokens × 1M; value = tokens ÷ cost (higher better).',
