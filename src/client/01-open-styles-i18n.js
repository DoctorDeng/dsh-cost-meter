...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = [
      ...
      // 插件定制主题色(浅蓝):余额/费用/Coding Plan/峰谷进度条统一引用宿主语义槽
      // --dsw-alias-state-business-primary,但宿主/主题给该槽的默认蓝偏深,故在这里
      // 把它重定向为浅蓝——作用域仅限下列插件元素(自定义属性可继承,故峰谷条的
      // 段/芯片/游标等后代元素一并生效),不污染宿主其它组件。改色只动此一处。
      '.cm-peak-strip,.cm-peak-classic,.cm-peak-rail,.cm-peak-rail-classic,.cm-peak-dot-circle,.cm-bbox-fill,.cm-bbox-pct.cm-bal-amt,.cm-tb-amt.cm-tb-bal,.cm-tb-pop-bal{--dsw-alias-state-business-primary:rgb(57,100,254)}',
