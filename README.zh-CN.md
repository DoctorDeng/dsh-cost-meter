...
**v1.7.34**：修复 web/headless 等多进程写账本互相覆盖，保留会话明细、每日合计和套餐用量。请更新所有使用此插件的 profile，并重启全部 DSH 进程。详见[更新说明](docs/release-notes/v1.7.34.md)。
...
**PowerShell 一键脚本**(复制整行粘贴回车;自动补齐 pnpm、自动探测 git,无需克隆仓库;安装链**固定到发布 tag `v1.7.34`**,建议先下载审阅再运行):
