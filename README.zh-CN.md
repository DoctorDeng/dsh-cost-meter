...
**v1.7.36**：修复 CDN 返回压缩数据却缺失 `Content-Encoding` 时 CommandCode 额度解析失败；请求默认协商明文响应，保留调用方显式指定的编码。详见[更新说明](docs/release-notes/v1.7.36.md)。
...
**PowerShell 一键脚本**(复制整行粘贴回车;自动补齐 pnpm、自动探测 git,无需克隆仓库;安装链**固定到发布 tag `v1.7.36`**,建议先下载审阅再运行):
