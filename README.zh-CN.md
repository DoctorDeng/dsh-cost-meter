...
**v1.7.33**：新增小米 MiMo Token Plan 原生额度支持——套餐/补偿积分窗口、周期截止(北京时间)重置与余额行,凭据为小米开放平台控制台 Cookie(tp-*/ttp-* 专用 Key 无用量端点)。详见[更新说明](docs/release-notes/v1.7.33.md)。
...
小米 MiMo Token Plan 的额度查询使用**控制台 Cookie**(而非 API Key):登录 `platform.xiaomimimo.com` 后按 F12 →「网络」→ 找到 `balanceAlertConfig` 请求,把请求头 `cookie` 整段粘贴进「设置 → 费用 → 额度」的 MiMo 卡片(需含 `serviceToken` 与 `userId`,DSH 凭据库以 `MIMO_COOKIE` 托管)。卡片显示套餐/补偿积分窗口与周期截止重置(北京时间)及余额行。控制台 Cookie 约 24 小时过期,卡片提示登录态过期时重新复制即可。Token Plan 推理专用 Key(`tp-*` / `ttp-*`)无用量端点,不能查询额度。
