...
**#140 的版本检查**：该报告使用的插件为 1.7.21。DSH Desktop 的版本号与插件版本不同，升级桌面程序不代表插件已更新；依赖声明 `^1.7.21` 也不能证明当前安装的是新版本。在实际使用的 DSH profile 依赖目录运行 `node -p "require('dsh-cost-meter/package.json').version"` 可查看已安装插件版本。若仍是旧版，按仓库 README 的安装命令更新插件，再完全退出并重启 DSH。无需把搜索记录改成 no-op；新版独立明细同时保留实时计费、历史导入与币种重算能力。
