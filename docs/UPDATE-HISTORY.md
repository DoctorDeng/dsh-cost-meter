...
## v1.7.39(2026-09-26)—— 额度缓存与计费稳定性修复

- 网关额度复用有效缓存与首次并发请求，查询失败保留上次成功结果；自定义余额修改地址、请求内容或条目顺序后及时刷新。
- 修改配置或凭据、关闭查询、卸载插件时取消失效请求，旧响应不能覆盖新状态；显示币种转换仍复用原始余额快照。
- 修复超过 1024 个会话后包装路由转发重复计费；异常账本保留原文件并恢复写入，外部用量拒绝不存在的日期和错误的来源结构。
- 新增 14 组针对性回归，完整回归、真实宿主热安装检查及客户端构建通过。升级后重启 DSH，并刷新浏览器。

## v1.7.38(2026-09-26)—— 外部用量来源

- 读取同账户其他进程写入的外部用量快照，在费用设置中按来源展示 DSH 与外部用量及日/月/累计合计。
- 支持准确费用日汇总或逐次调用记录；快照有大小、字段和时间限制，外部数据不混入 DSH 会话金额、预算及官方余额对账。格式见[外部用量说明](external-usage.md)。

## v1.7.37(2026-09-25)—— 中国法定假日全天谷价

- 中秋和国庆假期按北京日期全天使用 DeepSeek 谷价；周末即使调休上班仍按谷价。设置页可以编辑 `peakHolidays`，假日时段条显示独立标签，倒计时跳过假期内的无效切换点。
- 有完整会话日志的历史金额在升级时自动逐次重算；日志缺失或不匹配的记录保留原金额。
...
- 模型统计图可悬停查看完整模型名；裸 gzip 响应自动解压后解析，并保持大小限制。
- GLM 查询失败显示每个候选端点的具体阶段与状态，便于定位宿主内故障；宿主内失败原因仍待报告者复测。更新后重启 DSH 并刷新浏览器。

## v1.7.34(2026-09-22)—— 多进程账本合并

- #168：修复 web、headless 等多个 DSH 进程共享账本时互相覆盖会话及统计的问题；并发保存和常驻进程的后续保存都保留其他进程已写入的数据。
- 同步处理会话追加、匿名调用、套餐小时用量与采样；保留每日总量，不受 200 条会话明细上限影响。清空历史后，旧进程不会恢复此前的数据。
- 请在所有使用插件的 profile 中升级到本版本，再重启全部 DSH 进程。已丢失的会话可通过“导入安装前历史”从仍存在的宿主日志补回；已清理的日志无法回放。

## v1.7.33(2026-09-22)—— POST 请求体与小米 MiMo 额度

- #165：自定义余额选择 POST 后可填写 JSON 请求体，支持格式提示、清空及每条配置独立保存。
- JSON 原文发送，兼容旧对象/字符串配置；修正 Content-Type 大小写识别，补充双语示例与真实 HTTP、RPC、重启回归。
- #166：整合小米 MiMo Token Plan 额度查询，支持套餐/补偿积分、重置时间、余额和 Cookie 凭据托管；补齐重定向拒绝、取消请求及异常数字校验。

## v1.7.32(2026-09-22)—— 自定义余额按显示币种折算

- #162：在每条自定义余额配置中，开启“按显示币种折算（USD 余额）”，让余额与今日费用使用相同显示币种；默认关闭。
- 美元余额、已用金额和上限一起换算，原始数据与进度比例不变；其他源币种及积分保留原单位。支持设置保存、重启及旧单条配置。

## v1.7.31(2026-09-22)—— 币种提示、MiniMax 额度与缓存命中率

- #157：新增人民币对账提示和官方 CNY 选择入口，详见[币种说明](billing-currency.md)。
- #158：补齐 macOS / Linux 的 pnpm 安装与 PATH 排障；安装环境修复后重新启动 DSH。
- #159：MiniMax 无周限额显示 ∞，5h 卡片显示每分钟更新的重置倒计时。
- #160：会话底部直接显示缓存命中率；兼容无会话 ID、API / Plan 与子代理费用。
...
- 首屏读取失败会自动重试，也可查看具体错误并点击“刷新”；加载成功后恢复正常轮询。
- 回归覆盖真实宿主热安装、正常启动、服务晚到与卸载重装。详见[热安装说明](market-hot-install.md)。
...
- #143：额度区逐卡启用、展开与刷新，启用项自动置顶；保留原有配置、来源上限和重复条目，展开状态不持久化。整合 @UNscientific-9 的原始实现并补充排序、Go 设置与状态稳定性修复。
- 界面跟随中英文和币种设置；标准、简化、紧凑与 rail 侧栏均保留可用展示。
