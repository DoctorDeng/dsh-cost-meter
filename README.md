...
**v1.7.31** adds a DeepSeek CNY billing hint, MiniMax unlimited weekly quota (∞) and a visible 5h reset countdown, plus cache hit rate in the composer footer. macOS/Linux installation instructions now cover pnpm and PATH. See the [release notes](docs/release-notes/v1.7.31.md) and [billing currency guide](docs/billing-currency.md#english).
...
**PowerShell one-click script** (copy the whole line, paste, press Enter; pnpm is provisioned automatically, git is auto-detected — no clone needed; the install chain is **pinned to the release tag `v1.7.31`** — review the script before running):
