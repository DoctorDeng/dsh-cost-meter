...
**v1.7.23**：侧栏自定义余额靠右对齐，长名称自动省略、悬停可看全名。此前受原生搜索日志问题影响的会话，仍可按 [恢复说明](docs/session-history-recovery.md) 备份修复。
...
**PowerShell 一键脚本**(复制整行粘贴回车;自动补齐 pnpm、自动探测 git,无需克隆仓库;安装链**固定到发布 tag `v1.7.23`**,建议先下载审阅再运行):
