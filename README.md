...
**v1.7.26**：千问额度新增官方 CLI 来源，查询订阅 Credits 与加量包；Antigravity 可只显示 Gemini 额度；自定义余额支持小数积分与本机回环 HTTP。详见 [千问 CLI 使用说明](docs/qwen-cli-quota.md)。
...
**PowerShell 一键脚本**(复制整行粘贴回车;自动补齐 pnpm、自动探测 git,无需克隆仓库;安装链**固定到发布 tag `v1.7.26`**,建议先下载审阅再运行):
