...
**v1.7.29**：修复插件市场热安装后“账本不可用”、必须重启才恢复的问题；首屏读取失败会自动重试，并显示错误与刷新按钮。详见[热安装说明](docs/market-hot-install.md)。
...
**PowerShell 一键脚本**(复制整行粘贴回车;自动补齐 pnpm、自动探测 git,无需克隆仓库;安装链**固定到发布 tag `v1.7.29`**,建议先下载审阅再运行):
