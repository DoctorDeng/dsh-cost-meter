...
**v1.7.28**：MiniMax 额度查询默认优先使用 `www.minimax.cn`，新增可保存的 HTTPS 查询域名；手动指定后只访问该域名，更新插件保留配置。详见[配置说明](docs/minimax-quota-endpoint.md)。
...
**PowerShell 一键脚本**(复制整行粘贴回车;自动补齐 pnpm、自动探测 git,无需克隆仓库;安装链**固定到发布 tag `v1.7.28`**,建议先下载审阅再运行):
