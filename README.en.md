...
**v1.7.30**: OpenRouter models no longer cost $0 — a built-in 4-model snapshot backfills old ledgers, and startup / price sync / hourly refresh pulls the full catalog from `openrouter.ai/api/v1/models`, keeping the local snapshot on failure. See the [OpenRouter pricing guide](docs/openrouter-pricing.md#english).
...
| OpenRouter passthrough pricing | [pricing doc](docs/openrouter-pricing.md#english) | OpenRouter passes upstream prices through: built-in snapshot, automatic old-ledger backfill, public-catalog (no auth) refresh with additive merge, local snapshot kept on failure ([中文](docs/openrouter-pricing.md)) |
