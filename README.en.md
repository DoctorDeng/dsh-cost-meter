...
[![version](https://img.shields.io/badge/version-1.7.23-4176E6)](https://github.com/Han-1413141/dsh-cost-meter)

**v1.7.23** right-aligns custom balances in the sidebar. Long names use an ellipsis, with the full name available on hover. See the [recovery guide](docs/session-history-recovery.md#english) for sessions affected by the earlier native-search log issue.
...
**PowerShell one-click script** (copy the whole line, paste, press Enter; pnpm is provisioned automatically, git is auto-detected — no clone needed; the install chain is **pinned to the release tag `v1.7.23`** — review the script before running):
...
```sh
dsh plugin --profile web add github:Han-1413141/dsh-cost-meter#v1.7.23
```

Without git, use the GitHub tag archive:

```sh
dsh plugin --profile web add https://github.com/Han-1413141/dsh-cost-meter/archive/refs/tags/v1.7.23.tar.gz
```
