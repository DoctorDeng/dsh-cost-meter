...
/**
 * 压缩正文的解压兜底(issue #172;宿主 Brotli 响应是本次补上的第二种来源):
 * 服务端正文可能在没有可用 Content-Encoding 的情况下以压缩字节落地,直接进
 * JSON.parse 会报 "response body is not valid JSON"。两种已知来源:
 *  - 个别 CDN 边缘(实测 MiFE/3.4.34)返回 gzip 字节却不带 Content-Encoding/Content-Type;
 *  - DSH 宿主把 undici 的全局 dispatcher 换成另一大版本的实例(`dsh-http-proxy` 的
 *    `setGlobalDispatcher`,8.11.0 vs Node 内建 7.29.0)后响应头会在解码路径上丢失,
 *    而 api.commandcode.ai 的成功响应正是 `content-encoding: br` 的 chunked 流。
 * gzip/zlib 有魔数可判别,Brotli 没有,因此改为「先按原文解析,失败再逐个尝试解压」:
 * 正常路径仍只有一次 JSON.parse,不为未压缩正文付出解压代价。解压输出同样受 maxBytes
 * 约束(maxOutputLength),压缩体膨胀超限按 tooLarge 处理,防止小压缩体绕过读取上限;
 * 全部尝试失败抛不含正文的解析错误。
 */
const INFLATORS = [gunzipSync, brotliDecompressSync, inflateSync]

/** JSON.parse 结果包一层:区分「解析出 null」与「解析失败」。 */
const tryParseJson = text => {
  try { return { value: JSON.parse(text) } } catch { return null }
}

const parseBoundedBody = (buffer, maxBytes, tooLarge) => {
  const plain = tryParseJson(buffer.toString('utf8'))
  if (plain !== null) return plain.value
  for (const inflate of INFLATORS) {
    let decoded
    try { decoded = inflate(buffer, { maxOutputLength: maxBytes }) }
    catch (error) {
      if (error?.code === 'ERR_BUFFER_TOO_LARGE') throw tooLarge()
      continue
    }
    const parsed = tryParseJson(decoded.toString('utf8'))
...
export async function readJsonBounded(response, maxBytes = 262144) {
  ...
  if (response.body?.[Symbol.asyncIterator]) {
    ...
    return parseBoundedBody(Buffer.concat(chunks), maxBytes, tooLarge)
  }
  // 兼容没有流接口的 adapter/测试替身；标准 Node Response 均走上面的有界流读取。
  // 这条路径拿到的已是字符串（adapter 自行解码），无需也不应再做压缩兜底。
 ...
 * 额度/余额类请求一律索取明文正文。
 *
 * api.commandcode.ai 等 Cloudflare 前置端点在客户端广告 br 时以 `content-encoding: br`
 * 的 chunked 流响应。DSH 宿主会把 undici 的全局 dispatcher 换成另一大版本的实例
 * (`dsh-http-proxy` 的 `setGlobalDispatcher`),此后响应头会在解码路径上丢失,压缩正文
 * 未经解码就进入 JSON.parse,面板报「额度响应无法读取为有效 JSON」——端点本身完全正常。
 * 这类响应都是几百字节的小 JSON,压缩收益可忽略,直接索取 identity 从源头规避;
 * {@link readJsonBounded} 的压缩兜底仍作为服务端忽略该头时的第二道防线。
 */
