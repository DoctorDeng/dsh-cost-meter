...
import { OFFICIAL_PRICING_URL, OFFICIAL_PRICING_URL_ZH, LEGACY_BASE_BOUNDARY, FLASH_PRICE_EFFECTIVE_AT, PRO_FLASH_ROUTING_EFFECTIVE_AT, OPENROUTER_MODELS_URL, parseOpenRouterModels, normalizePrice, parsePricingHtml, repairDefaultPeakPrice, upgradeDeepSeekPriceTable, costOf, providerPriceEntryFor, buildPriceCatalog, usdFromCost, isWrapperProviderId, wrapperUpstreamProvider } from './pricing.js'
...
async function buildState(ledger, balance = emptyBalance(), goQuota = emptyGoQuota(), codingPlans = {}, customBalance = emptyCustomBalance(), reconcile = { ok: true, message: '' }, ctx = null, customBalances = [], gatewayQuotas = []) {
  ...
  const planStats = buildPlanStats({
    ...
    // CLI exposes current credits but no reliable quota-period start/reset.
    codingPlans: ledger.config.codingPlans?.qwen?.quotaSource === 'cli' ? { ...codingPlans, qwen: undefined } : codingPlans,
    goQuota,
...
/** 从 OpenRouter 模型目录刷新 providers.openrouter.models(Phase2 实时刷新)。
 *  fail-soft:调用方自行 catch,失败不影响官方页同步的 ok 状态。
 *  合并语义:逐条 normalizePrice 校验后**加法合并**——保留未被刷新覆盖的
 *  旧 id(含本地 4 模型快照与离线期条目),同 id 用远端值替换。
 *  计价口径:OpenRouter 返回 USD/token,flat 条目恒为美元(usdFromCost 恒等),
 *  与账本美元记账一致,**不**随显示币种折算,直接原样入库。
 *  @returns {{ count }} 成功合入的模型数;0 条或解析失败时抛错。 */
async function refreshOpenRouterPrices(ledger) {
  const response = await fetchWithRetry(OPENROUTER_MODELS_URL, {
    headers: { 'user-agent': 'dsh-cost-meter/0.4 (DeepSeek Harness plugin)' },
  }, { timeoutMs: 20000 })
  if (!response.ok) throw new Error(`HTTP ${String(response.status)}`)
  // 目录载荷约 735KB,默认 256KB 上限不够,显式放宽到 4MB。
  const json = await readJsonBounded(response, 4 * 1024 * 1024)
  const parsed = parseOpenRouterModels(json)
  const prevModels = ledger.config.prices?.providers?.openrouter?.models ?? {}
  ...
  const { config, errors } = applyConfigPatch(ledger.config, {
    prices: {
      ...ledger.config.prices,
      providers: {
        ...ledger.config.prices?.providers,
        openrouter: { ...ledger.config.prices?.providers?.openrouter, models: merged },
      },
    },
  })
 ...
 *  永不抛错(不翻转官方同步的 ok 状态)。 */
async function openRouterRefreshNote(ledger, locale) {
  try {
    const { count } = await refreshOpenRouterPrices(ledger)
...
function createService(ctx, ledger) {
  ...
  ctx.effect(() => () => {
    codingPlanCaches.qwen?.controller?.abort()
    delete codingPlanCaches.qwen
    codingPlanCaches.minimax?.controller?.abort()
    delete codingPlanCaches.minimax
  ...
  const mergedCodingPlans = async () => {
    ...
    for (const id of CODING_PLAN_PROVIDER_IDS) {
      ...
      out[id] = {
        ...
        ...(id === 'qwen' ? { quotaSource: cfg.quotaSource === 'cli' ? 'cli' : 'local' } : {}),
      }
    }
    return out
  }
  ...
  const ensureQwenPlan = async force => {
    ...
    cache.inFlight = queryQwenCli(locale, { signal: cache.controller.signal }).then(result => {
  ...
  const service = {
    ...
    async fetchPrices() {
      ...
      try {
        ...
        const orNote = await openRouterRefreshNote(ledger, locale)
...
export async function runStartupImports(ledger, sessionsRoot) {
  ...
  // OpenRouter 快照定价回填:openrouter vendor 价格表新增后，旧账本里 $0 的
  // openrouter:* 桶需要按新快照重算一次（pricing-go-fix-v1 已在旧价格表下
  // 跑过，不会重跑）。幂等，阈值防抖。
  if (!ledger.migrations.includes('pricing-openrouter-v1')) {
    const repaired = repairLedgerPricing(ledger)
    ledger.migrations.push('pricing-openrouter-v1')
    ledger.scheduleWrite()
    if (repaired.recostedBuckets > 0) {
      console.log(`[dsh-cost-meter] OpenRouter 定价回填完成:重算 ${repaired.recostedBuckets} 个模型桶 / ${repaired.touchedDays} 天 / ${repaired.touchedSessions} 个会话`)
    }
    if (repaired.recostedBuckets > 0) splitLedgerApiCost(ledger)
  }
...
export function apply(ctx) {
  ...
  const backfillTimer = setTimeout(async () => {
    ...
    // OpenRouter 全量价目启动刷新(Phase2):失败只告警,本地快照继续兜底。
    refreshOpenRouterPrices(ledger).then(({ count }) => {
      console.log(`[dsh-cost-meter] OpenRouter 价格启动刷新:已合入 ${String(count)} 个模型`)
    }).catch(error => {
      console.warn(`[dsh-cost-meter] OpenRouter 价格启动刷新失败(保留本地快照): ${String(error?.message ?? error)}`)
    })
  }, 3000)
  backfillTimer.unref?.()
  // 卸载/退出时清掉尚未触发的启动回填定时器:dispose 后不再执行导入。
  ctx.effect(() => () => clearTimeout(backfillTimer), 'cost-meter: backfill timer')
  // OpenRouter 全量价目每小时刷新(Phase2):只影响未来计费,不重算历史。
  const openRouterTimer = setInterval(() => {
    refreshOpenRouterPrices(ledger).then(({ count }) => {
      console.log(`[dsh-cost-meter] OpenRouter 价格定时刷新:已合入 ${String(count)} 个模型`)
    }).catch(error => {
      console.warn(`[dsh-cost-meter] OpenRouter 价格定时刷新失败(保留本地快照): ${String(error?.message ?? error)}`)
    })
  }, 3600_000)
