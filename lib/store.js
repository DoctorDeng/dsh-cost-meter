...
export function applyConfigPatch(current, rawPatch) {
  ...
  for (const [id, raw] of Object.entries(candidate.codingPlans)) {
    ...
    if (id === 'qwen') {
      entry.quotaSource = entry.quotaSource === 'cli' ? 'cli' : 'local'
      const credits = Number(entry.planCredits)
  ...
  if (candidate.peakStyle !== 'compact' && candidate.peakStyle !== 'classic' && candidate.peakStyle !== 'dot') errors.push(vmsg(locale, 'peakStyle'))
...
export function sanitizeConfig(raw) {
  ...
  out.peakStyle = oneOf(out.peakStyle, ['compact', 'classic', 'dot'], 'compact')
  ...
  if (out.codingPlans !== null && typeof out.codingPlans === 'object') {
    for (const [id, entry] of Object.entries(out.codingPlans)) {
      ...
      if (id === 'qwen') {
        plans[id].quotaSource = entry.quotaSource === 'cli' ? 'cli' : 'local'
        const credits = Number(entry.planCredits)
...
export class Ledger {
  ...
  static open() {
    ...
    const ledger = new Ledger(config, days, path, migrations, { planSamples, planHourBuckets, openrouterPriceHashes })
  ...
  flush() {
    ...
    try {
      ...
      writeFileSync(tmp, JSON.stringify({
        ...
        openrouterPriceHashes: this.openrouterPriceHashes,
      }), 'utf8')
