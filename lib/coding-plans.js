...
export const CODING_PLAN_PROVIDERS = {
  ...
  mimo: {
    ...
    // serviceToken)与 userId 字段,约 24 小时过期,需重新复制)。
    credentialEnvs: ['MIMO_COOKIE', 'XIAOMI_MIMO_COOKIE'],
    keyHint: '小米开放平台控制台 Cookie(登录 platform.xiaomimimo.com 后 F12→网络→balanceAlertConfig→请求头 cookie 整段复制;约 24 小时过期)',
  },
}
...
// 端点与响应结构参照 CodexBar 的 MiMoUsageFetcher 与 cc-switch 社区脚本实测):
...
export function parseMimoBalance(data) {
  if (data === null || typeof data !== 'object') return null
  const d = data.data !== null && typeof data.data === 'object' ? data.data : data
  const n = Number(d.balance ?? d.cashBalance)
  if (!Number.isFinite(n) || n < 0) return null
 ...
 */
export async function queryCodingPlan(provider, key, locale, t, { baseUrl, signal } = {}) {
  ...
  if (provider === 'mimo') {
    ...
    const readJson = async url => {
      ...
      const response = await fetchWithRetry(url, { headers }, { timeoutMs: 15000, attempts: 2 })
      if (response.status === 401 || response.status === 403) {
        const error = new Error(t(locale, 'mimoLoginExpired', { provider: meta.label }))
        error.soft = true
        throw error
      }
      if (!response.ok) throw new Error(t(locale, 'codingPlanHttp', { provider: meta.label, code: String(response.status), url }))
  ...
  for (const url of urls) {
    ...
    try {
      ...
      response = await fetchWithRetry(url, { headers, ...(provider === 'minimax' ? { redirect: 'error', signal } : {}) }, { timeoutMs: 15000, attempts: 2 })
    } catch (error) {
      lastError = error
    ...
    let data
    try { data = provider === 'minimax' ? await readJsonBounded(response) : await response.json() } catch { data = null }
