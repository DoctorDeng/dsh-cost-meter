...
export function repairDefaultPeakPrice(prices) {
  ...
  const expected = (prices?.currency === 'CNY' ? DEFAULT_PRICE_TABLE_CNY : DEFAULT_PRICE_TABLE).default
  const fields = ['cacheHit', 'cacheMiss', 'output']
  if (!current || Object.keys(current).some(key => !fields.includes(key))
    || !fields.every(key => current[key] === expected[key])) return false
