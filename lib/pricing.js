...
export const DEFAULT_PROVIDER_PRICE_TABLE = {
  ...
  // OpenRouter 网关按原样透传上游价(https://openrouter.ai/api/v1/models)。
...
export function parseOpenRouterModels(json) {
  ...
  const perMillion = (usdPerToken) => Math.round(usdPerToken * 1e12) / 1e6
  const models = {}
  for (const item of list) {
    ...
    const raw = {
      input: perMillion(prompt),
      output: perMillion(completion),
      billingMode: 'flat',
      sourceUrl: OPENROUTER_MODELS_URL,
      checkedAt: today,
    }
    const cacheRead = rate(pricing.input_cache_read)
    if (cacheRead !== undefined) raw.cachedInput = perMillion(cacheRead)
    const cacheWrite = rate(pricing.input_cache_write)
    if (cacheWrite !== undefined) raw.cacheWrite = perMillion(cacheWrite)
    models[id] = raw
  }
  return { models }
}
