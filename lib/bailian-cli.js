...
function windowsOf(payload, locale) {
  const windows = {}
  for (const [rawKey, name] of PERIOD_KEYS) {
    const cell = payload[rawKey]
    if (cell === undefined) continue
    if (!record(cell)) throw fail(locale, 'invalid')
    const total = Number(cell.totalQuota)
    // Unbounded or missing limits produce no window (the console prints "unlimited").
    if (!Number.isFinite(total) || total <= 0) continue
    const rawPct = cell.percentage !== undefined && Number.isFinite(Number(cell.percentage))
      ? Number(cell.percentage)
      : Number(cell.usedQuota) / total * 100
    if (!Number.isFinite(rawPct)) continue
    const ms = Number(cell.resetTime)
    windows[name] = {
      percent: Math.max(0, Math.min(100, Math.round(rawPct * 10) / 10)),
      resetsAt: Number.isFinite(ms) && ms > 0 ? new Date(ms).toISOString() : '',
    }
  }
  return windows
}
...
export function parseBailianUsage(payloads, locale = 'zh') {
  ...
  for (const value of [token, coding]) {
    if (value !== null && value !== undefined && !record(value)) throw fail(locale, 'invalid')
  }
  const tokenWins = token ? windowsOf(token, locale) : {}
  const codingWins = coding ? windowsOf(coding, locale) : {}
  const windows = {}
  for (const name of WINDOW_ORDER) {
    const win = tokenWins[name] ?? codingWins[name]
    if (win !== undefined) windows[name] = win
  }
  if (Object.keys(windows).length === 0) throw fail(locale, 'unavailable')
  const sources = []
