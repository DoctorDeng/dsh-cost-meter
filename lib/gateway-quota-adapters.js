...
export function parseAntigravityQuota(payload, ctx = {}) {
  ...
  for (let gi = 0; gi < groups.length; gi++) {
    ...
    // geminiOnly(来源配置 antigravityOnlyGemini):只留 Gemini 原生组,第三方模型池
    // (Google 侧 "Claude and GPT models")整组跳过——它消耗同一订阅额度但不是 Gemini。
    if (geminiOnly && /claude|gpt/i.test(groupTitle)) {
      warnings.push(`antigravity: 第三方模型池「${groupTitle}」已按 geminiOnly 过滤`)
      continue
    }
